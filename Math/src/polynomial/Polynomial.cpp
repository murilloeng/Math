...
namespace math
{
	...
	//data
	uint32_t Polynomial::order(void) const
	{
		return m_order;
	}
	uint32_t Polynomial::order(uint32_t order)
	{
		delete[] m_roots;
		delete[] m_coefficients;
	...
	const std::complex<double> Polynomial::root(uint32_t index) const
	{
		return m_roots[index];
	}

	std::complex<double> Polynomial::coefficient(uint32_t index) const
	{
		return m_coefficients[index];
	}
	std::complex<double> Polynomial::coefficient(uint32_t index, std::complex<double> coefficient)
	{
	...
	bool Polynomial::solve(void)
	{
		bound();
		predictor();
		for(uint32_t i = 0; i < m_iteration_max; i++)
		{
			corrector();
