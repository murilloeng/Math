...
{
	//constructors
	span::span(matrix& k, unsigned row, unsigned col, unsigned rows, unsigned cols) : 
	...
	double& span::operator()(unsigned i, unsigned j) const
	{
	...
	matrix operator*(const span& s, const matrix& k)
	{
		...
		for(unsigned i = 0; i < s.m_rows; i++)
		{
			for(unsigned j = 0; j < k.m_cols; j++)
			{
				r(i, j) = 0;
				for(unsigned p = 0; p < s.m_cols; p++)
				{
					r(i, j) += s(i, p) * k(p, j);
				}
			}
	...
	matrix operator*(const matrix& k, const span& s)
	{
		...
		for(unsigned i = 0; i < k.m_rows; i++)
		{
			for(unsigned j = 0; j < s.m_cols; j++)
			{
				r(i, j) = 0;
				for(unsigned p = 0; p < k.m_cols; p++)
				{
					r(i, j) += k(i, p) * s(p, j);
				}
			}
